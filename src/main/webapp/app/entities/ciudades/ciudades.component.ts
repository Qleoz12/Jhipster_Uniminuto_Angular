...
export class CiudadesComponent implements OnInit, OnDestroy {
  ...
  trackId(index: number, item: ICiudades) {
    return item.id;
  }

  registerChangeInCiudades() {
    this.eventSubscriber = this.eventManager.subscribe('ciudadesListModification', response => this.loadAll());
  }
  ...
  protected paginateCiudades(data: ICiudades[], headers: HttpHeaders) {
